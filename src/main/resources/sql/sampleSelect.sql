...
-- SQL - pobiera najnowszy plan dla zadanego użytkownika (tabela admins)
SELECT * FROM recipe_plan JOIN day_name on day_name.id=day_name_id JOIN recipe on recipe.id=recipe_id WHERE
plan.plan_id =  (SELECT MAX(id) from plan WHERE admin_id = 1) ORDER by day_name.display_order, recipe_plan.display_order;

-- SQL - pobiera najnowszy plan dla zadanego użytkownika (tabela admins)
SELECT * FROM plan JOIN day_name on day_name.id=day_name_id JOIN recipe on recipe.id=recipe_id WHERE
recipe_plan.plan_id =  (SELECT MAX(id) from plan WHERE admin_id = 1) 
ORDER by day_name.display_order, recipe_plan.display_order;
